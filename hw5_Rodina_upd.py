...
class Graph:
    ...
    def compress(self):
        ...
        for vert in self.vertices.keys():
            if (len(self.vertices[vert].out_edges) == 1) and (len(self.vertices[vert].in_edges) == 1):
                self.comr_vert.append(vert)

        for vert in self.comr_vert:
            if vert in self.vertices and len(self.vertices) > 2:
                prev = list(self.vertices[vert].in_edges.keys())[0]
                next = list(self.vertices[vert].out_edges.keys())[0]
                new_edge = Edge(prev, next)
                self.vertices[prev].out_edges[next] = [new_edge]
                self.vertices[next].in_edges[prev] = [new_edge]
                new_edge.seq = self.vertices[vert].in_edges[prev][0].seq + self.vertices[next].in_edges[vert][0].seq[self.k:]
                new_edge.coverage = (self.vertices[prev].out_edges[vert][0].coverage + self.vertices[next].in_edges[vert][0].coverage)/2
                del self.vertices[prev].out_edges[vert]
                del self.vertices[next].in_edges[vert]
...
my_graph.graph_vis('graph_full_compressed', status='f')
